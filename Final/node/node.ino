...
#define LORA_DIO0    2    // LoRa DIO0                                 2
...
#define I2C_SCL      22    // BME280 + MPU6050 SCL                      22
...
void setup() {
  ...
  server.on("/simulate_alert", HTTP_GET, []() {
    ...
    setLed(1, 0, 1);  // purple for seismic
    server.sendHeader("Access-Control-Allow-Origin", "*");
...
void loop() {
  ...
  // ── LED recovery: after 3 s return to green ────────
  static unsigned long alertLedUntil = 0;
  if (simulatedAlertActive || (millis() - lastButtonPress < 3000)) {
    alertLedUntil = millis() + 3000;
  }
